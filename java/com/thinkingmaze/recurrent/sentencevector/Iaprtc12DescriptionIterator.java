...
public class Iaprtc12DescriptionIterator implements DataSetIterator {
	...
	public DataSet next(int num) {
		...
		INDArray input = Nd4j.zeros(exampleLength+numCharacters, numExamplesToFetch);
		INDArray labels = Nd4j.zeros(numCharacters, numExamplesToFetch);
		num = 1;
		for (int i = 0; i < num; i++){
			...
			for (int j = 0; j<numExamplesToFetch && j+featureLength+imageV<imageAndDescription.size(); j++){
				input.putScalar(new int[]{preChar, j}, 1.0f);
				Writable current = imageAndDescription.get(featureLength+imageV+j);
				labels.putScalar(new int[]{current.toInt(), j}, 1.0f);
